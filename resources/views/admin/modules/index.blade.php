...
@section('content')
  ...
  <!-- 1. DESKTOP VIEW: Clean Light Minimal Table (Visible only on md+ / Desktop) -->
      ...
      <tbody>
        @forelse($modules as $module)
          <tr>
            ...
            <td>
              <strong class="cell-truncate" title="{{ $module->title }}" style="color: var(--color-text-primary, #0f172a); font-size: 0.90rem; font-weight: 700; display: block; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
                {{ $module->title }}
              </strong>
            </td>
            <td>
            ...
            <td>
              <p class="cell-truncate" title="{{ $module->description }}" style="color: var(--color-text-secondary, #475569); font-size: 0.82rem; line-height: 1.4; margin: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
                {{ $module->description }}
              </p>
            </td>
            <td style="text-align: center;">
              <div style="display: flex; align-items: center; justify-content: center; gap: 6px;">
                <a href="{{ route('admin.modules.edit', $module->id) }}" style="display: inline-flex; align-items: center; gap: 5px; background: var(--color-primary-subtle, #eaf2ff); border: 1px solid #bfdbfe; color: var(--color-primary, #2563eb); padding: 6px 12px; border-radius: 6px; font-size: 0.76rem; font-weight: 600; text-decoration: none; white-space: nowrap; transition: all 0.15s;" onmouseover="this.style.background='#2563eb'; this.style.color='#ffffff'" onmouseout="this.style.background='var(--color-primary-subtle, #eaf2ff)'; this.style.color='var(--color-primary, #2563eb)'" title="Edit Modul">
  ...
  <div class="admin-cards-mobile block md:hidden">
    ...
    @empty
      <div class="admin-empty-card">
        ...
        <p style="margin: 0; font-size: 0.95rem; font-weight: 600; color: var(--color-text-primary, #0f172a);">Belum ada modul terdaftar.</p>
        <p style="margin: 4px 0 16px; font-size: 0.82rem; color: var(--color-text-secondary, #64748b);">Klik tombol di bawah untuk menambahkan modul pertama.</p>
